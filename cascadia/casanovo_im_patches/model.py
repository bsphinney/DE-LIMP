"""A de novo peptide sequencing model with optional ion mobility support."""
...
class Spec2Pep(pl.LightningModule, ModelMixin):
    ...
    use_ion_mobility : bool
        Whether to use ion mobility (1/K0) as an additional input feature.
        When True, the precursor 1/K0 value is encoded and added to the
        spectrum encoder output. Default False for backward compatibility.
    **kwargs : Dict
    ...
    def __init__(
        ...
        **kwargs: Dict,
        ...
        # Exclude non-picklable objects from hyperparameters
        self.save_hyperparameters(ignore=["out_writer", "tb_summarywriter"])
        ...
        # Ion mobility encoding components.
        self.use_ion_mobility = use_ion_mobility
        if self.use_ion_mobility:
            # FloatEncoder maps a single float (1/K0) to dim_model features
            # using sinusoidal encoding. 1/K0 range is typically 0.6-1.7 for
            # tryptic peptides.
            self.im_encoder = FloatEncoder(
                dim_model=dim_model,
                min_wavelength=0.001,
                max_wavelength=2.0,
            )
            # Linear projection to combine spectrum encoding + IM encoding
            self.im_projector = torch.nn.Linear(
                2 * dim_model, dim_model, bias=False
            )
    ...
    def _encode_spectra(
        ...
        """
        Encode spectra, optionally incorporating ion mobility information.

        Parameters
        ----------
        spectra : torch.Tensor of shape (n_spectra, n_peaks, 2)
            The spectra to encode.
        precursors : torch.Tensor of shape (n_spectra, 3) or (n_spectra, 4)
            Precursor info: (mass, charge, m/z) or (mass, charge, m/z, im).

        Returns
        -------
        memories : torch.Tensor of shape (n_spectra, n_peaks + 1, dim_model)
            The encoded spectra.
        ...
        if self.use_ion_mobility and precursors.shape[1] >= 4:
            # Extract 1/K0 values: shape (n_spectra,)
            im_values = precursors[:, 3]

            # Encode IM: FloatEncoder expects (batch, seq_len) input
            # We broadcast the single IM value across all peaks+1 positions
            im_encoded = self.im_encoder(
                im_values[:, None].expand(-1, memories.shape[1])
            )
            # im_encoded shape: (n_spectra, n_peaks+1, dim_model)

            # Concatenate spectrum encoding with IM encoding and project back
            combined = torch.cat([memories, im_encoded], dim=2)
            memories = self.im_projector(combined)
    ...
    def forward(
        ...
        precursors : torch.Tensor of size (n_spectra, 3) or (n_spectra, 4)
            The measured precursor mass (axis 0), precursor charge (axis 1),
            precursor m/z (axis 2), and optionally 1/K0 (axis 3).
    ...
    def beam_search_decode(
        ...
        -------
        pred_peptides : List[List[Tuple[float, np.ndarray, str]]]
            For each spectrum, a list with the top peptide prediction(s).
        """
        # Use IM-aware encoding
        memories, mem_masks = self._encode_spectra(spectra, precursors)
        ...
        # For the decoder, use only (mass, charge, mz) — first 3 columns
        precursors_3col = precursors[:, :3]
        ...
        pred, _ = self.decoder(None, precursors_3col, memories, mem_masks)
        ...
        precursors_3col = einops.repeat(
            precursors_3col, "B L -> (B S) L", S=beam
        )
        ...
        for step in range(0, self.max_length):
            ...
            (
                ...
                beam_fits_precursor,
                discarded_beams,
            ) = self._finish_beams(tokens, precursors_3col, step)
            ...
            scores[~finished_beams, : step + 2, :], _ = self.decoder(
                ...
                precursors_3col[~finished_beams, :],
    ...
    def _finish_beams(
        ...
        precursors : torch.Tensor of shape (n_spectra * n_beams, 3)
            Precursor info (mass, charge, m/z).
        step : int
    ...
    def _forward_step(
        ...
        # Use IM-aware encoding
        memories, mem_masks = self._encode_spectra(spectra, precursors)
        # Decoder uses only (mass, charge, mz)
        precursors_3col = precursors[:, :3]
        return self.decoder(sequences, precursors_3col, memories, mem_masks)
    ...
    def predict_step(
        ...
        predictions: List[Tuple[np.ndarray, float, float, str, float, np.ndarray]]
            Model predictions for the given batch of spectra.
        """
        predictions = []
        for (
            precursor_charge,
    ...
    def from_pretrained(
        cls,
        checkpoint_path: str,
        use_ion_mobility: bool = True,
        **override_kwargs,
    ):
        """Load a pre-trained checkpoint and upgrade to IM-enabled model.

        Creates a new Spec2Pep with use_ion_mobility=True, loads the old
        weights, and zero-initializes the IM channel so the model produces
        identical output to the original when IM values are zero.

        Parameters
        ----------
        checkpoint_path : str
            Path to the .ckpt file from the original (non-IM) model.
        use_ion_mobility : bool
            Whether to enable the IM channel (default True).
        **override_kwargs
            Any additional keyword args to override from the checkpoint's
            hyper_parameters (e.g., lr, n_beams).

        Returns
        -------
        Spec2Pep
            The model with pre-trained weights loaded and IM components
            zero-initialized.
        """
        ...
        # Build constructor kwargs from checkpoint hparams + overrides
        constructor_kwargs = dict(hparams)

        # Remove non-constructor params that may be in hparams
        for key in list(constructor_kwargs.keys()):
            if key in ("out_writer", "tb_summarywriter"):
                constructor_kwargs.pop(key)

        constructor_kwargs["use_ion_mobility"] = use_ion_mobility
        constructor_kwargs.update(override_kwargs)

        # Create new model with IM support
        model = cls(**constructor_kwargs)

        # Load old state dict, skipping IM-specific keys
        ...
        for name, param in old_state.items():
            if name in new_state and new_state[name].shape == param.shape:
                ...
                logger.warning(
                    ...
                    "skipping (will use initialized weights)",
                    name,
                    param.shape,
                    new_state[name].shape,
                )
            else:
                logger.info(
                    "Key %s from checkpoint not in new model, skipping", name
                )

        # Zero-initialize IM-specific components for backward compatibility
        if use_ion_mobility:
            # Zero the projector weights so IM channel contributes nothing
            # when IM=0 (sinusoidal encoding of 0 is non-zero, but the
            # projection output is zeroed).
            # Strategy: zero out the IM half of im_projector weights
            # im_projector.weight shape: (dim_model, 2*dim_model)
            # First dim_model columns = spectrum encoding (keep from old)
            # Last dim_model columns = IM encoding (zero-init)
            proj_key = "im_projector.weight"
            if proj_key in new_state:
                dim_model = new_state[proj_key].shape[0]
                # Initialize as identity for spectrum part, zero for IM part
                new_state[proj_key].zero_()
                # Copy identity for the spectrum encoding columns
                new_state[proj_key][:, :dim_model] = torch.eye(dim_model)

        model.load_state_dict(new_state)

        logger.info(
            "Loaded pre-trained weights from %s with use_ion_mobility=%s",
            checkpoint_path,
            use_ion_mobility,
        )
        return model
