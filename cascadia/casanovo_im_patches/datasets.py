"""A PyTorch Dataset class for annotated spectra with optional ion mobility."""
...
class IMSpectrumDataset(SpectrumDataset):
    """
    A SpectrumDataset that also returns ion mobility (1/K0) values.

    The ion mobility values are loaded from an external mapping (im_array)
    that parallels the spectrum index. This avoids modifying depthcharge's
    HDF5 index format.
    ...
    im_array : numpy.ndarray of shape (n_spectra,)
        The 1/K0 values for each spectrum, in the same order as the index.
        If None, all IM values default to 0.0.
    n_peaks : int
        The number of top-n most intense peaks to keep.
    min_mz : float
        The minimum m/z to include.
    max_mz : float
        The maximum m/z to include.
    min_intensity : float
        Remove peaks below this fraction of base peak.
    remove_precursor_tol : float
        Remove peaks near precursor m/z (Da).
    random_state : Optional[int]
    ...
    def __getitem__(
        self, idx
    ) -> Tuple[torch.Tensor, float, int, float, Tuple[str, str]]:
        """
        Return the MS/MS spectrum with ion mobility.
        ...
        im = float(self.im_array[idx]) if self.im_array is not None else 0.0
        return (
            spectrum,
            precursor_mz,
            precursor_charge,
...
class IMAnnotatedSpectrumDataset(AnnotatedSpectrumDataset):
    """
    An AnnotatedSpectrumDataset that also returns ion mobility (1/K0) values.

    Parameters
    ----------
    annotated_spectrum_index : depthcharge.data.SpectrumIndex
        The annotated MS/MS spectra to use as a dataset.
    im_array : numpy.ndarray of shape (n_spectra,)
        The 1/K0 values for each spectrum, in the same order as the index.
        If None, all IM values default to 0.0.
    n_peaks : int
    ...
    def __getitem__(
        self, idx: int
    ) -> Tuple[torch.Tensor, float, int, float, str]:
        """
        Return the annotated MS/MS spectrum with ion mobility.
        ...
        im = float(self.im_array[idx]) if self.im_array is not None else 0.0
        return spectrum, precursor_mz, precursor_charge, im, peptide
