...
class AsfParser(BaseParser):
    ...
    Supports both 4-column (mz, intensity, rt, level) and 5-column
    (mz, intensity, rt, level, im) ASF formats. When the 5th column
    (ion mobility / 1/K0) is present, it is stored in im_arrays and
    passed to MassSpectrum objects.
    ...
    def read(self) -> BaseParser:
        ...
        with open(self.path) as asf:
            for line in tqdm(asf, desc=str(self.path), unit="lines"):
                ...
                if line[0] == 'BEGIN':
                    ...
                    # 4-column: mz intensity rt level
                    # 5-column: mz intensity rt level im
                    mzs.append(float(line[0]))
                    intensities.append(float(line[1]))
                    rts.append(float(line[2]))
                    levels.append(float(line[3]))
                    if len(line) >= 5:
                        ims.append(float(line[4]))
                        spectrum_has_im = True
                    else:
                        ims.append(0.0)
                elif line[0] == 'END':
                    ...
                    # Track whether any spectrum in the file has IM data
                    if spectrum_has_im:
                        self.has_im = True
                    ...
                    spectrum = MassSpectrum(
                        ...
                        im=np.array(ims) if self.has_im or spectrum_has_im else None,
                        precursor_mz=precursor_mz,
                        precursor_charge=precursor_charge,
                        label=label,
                    ...
                    self.im_arrays.append(
                        spectrum.im if spectrum.im is not None
                        else np.zeros(len(spectrum.mz), dtype=np.float32)
                    )
        ...
        self.im_arrays = np.concatenate(self.im_arrays).astype(np.float32)
