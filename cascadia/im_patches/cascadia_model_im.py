...
class AugmentedPeakEncoder(torch.nn.Module):
    """Encode an augmented m/z, intensity, RT, MS level, and optionally ion mobility.
    ...
    min_rt_wavelength : float, optional
        The minimum wavelength to use for RT.
    max_rt_wavelength : float, optional
        The maximum wavelength to use for RT.
    ...
    max_intensity_wavelength : float, optional
        ...
        intensities between [0, 1].
    use_ion_mobility : bool, optional
        Whether to use ion mobility (1/K0) as a 4th encoding channel.
    min_im_wavelength : float, optional
        The minimum wavelength for ion mobility encoding.
    max_im_wavelength : float, optional
        The maximum wavelength for ion mobility encoding.
    """
    ...
    def __init__(
        ...
        min_im_wavelength: float = 0.001,
        max_im_wavelength: float = 2.0,
    ) -> None:
        """Initialize the AugmentedPeakEncoder."""
        super().__init__()
        ...
        if self.use_ion_mobility:
            self.im_encoder = FloatEncoder(
                d_model,
                min_wavelength=min_im_wavelength,
                max_wavelength=max_im_wavelength
            )
            self.combiner = torch.nn.Linear(4 * d_model, d_model, bias=False)
        else:
            self.combiner = torch.nn.Linear(3 * d_model, d_model, bias=False)

    def forward(self, X: torch.Tensor) -> torch.Tensor:
        """Encode m/z values, intensities, rts, mslevel, and optionally ion mobility.

        Note that we expect intensities to fall within the interval [0, 1].

        Parameters
        ----------
        X : torch.Tensor of shape (n_spectra, n_peaks, 4 or 5)
            The spectra to embed. Axis 0 represents a mass spectrum, axis 1
            contains the peaks in the mass spectrum, and axis 2 is a tuple
            specifying (m/z, intensity, retention time, ms level[, ion_mobility])
            for each peak. These are zero-padded, such that all of the spectra
            in the batch are the same length.
        ...
        if self.use_ion_mobility:
            # Handle both 4-col (no IM) and 5-col (with IM) input tensors
            if X.shape[2] >= 5:
                im_values = X[:, :, 4]
            else:
                # No IM column present -- use zeros
                im_values = torch.zeros(
                    X.shape[0], X.shape[1],
                    device=X.device, dtype=X.dtype
                )
            channels.append(self.im_encoder(im_values))

        encoded = torch.cat(channels, dim=2)
        return self.combiner(encoded)

    def load_pretrained_weights(self, old_state_dict: dict) -> None:
        """Load weights from a pre-trained 3-channel model into this 4-channel model.

        Copies all shared weights (peak_encoder, rt_encoder, level_encoder) directly.
        For the combiner, copies the old 3*d_model columns and zero-initializes
        the new d_model columns (IM channel), so the model starts identical to
        the pre-trained model when IM input is zero.

        Parameters
        ----------
        old_state_dict : dict
            State dict from a 3-channel AugmentedPeakEncoder.
        """
        ...
        for name, param in old_state_dict.items():
            ...
            if name == "combiner.weight":
                # old shape: (d_model, 3*d_model), new shape: (d_model, 4*d_model)
                # Copy old weights into first 3*d_model columns, zero the rest
                d_model = param.shape[0]
                old_width = param.shape[1]  # 3 * d_model
                own_state[name].zero_()
                own_state[name][:, :old_width].copy_(param)
            else:
...
class AugmentedSpec2Pep(pl.LightningModule):
    def __init__(
        ...
        self.peak_encoder = AugmentedPeakEncoder(
            ...
            use_ion_mobility=use_ion_mobility,
        )
    ...
    @classmethod
    def from_pretrained(cls, checkpoint_path, use_ion_mobility=True, **override_kwargs):
        """Load a pre-trained 3-channel checkpoint into the new 4-channel architecture.

        This creates a new model with IM support and loads the old weights,
        zero-initializing the IM channel so the model produces identical output
        to the original when IM values are zero.

        Parameters
        ----------
        checkpoint_path : str
            Path to the .ckpt file from the original 3-channel model.
        ...
        **override_kwargs
            ...
            hyper_parameters (e.g., lr, dropout). Must include 'tokenizer'
            if not stored in the checkpoint.

        Returns
        -------
        AugmentedSpec2Pep
            The model with pre-trained weights loaded.
        """
        # Load checkpoint to extract hyperparameters and state dict
        checkpoint = torch.load(checkpoint_path, map_location="cpu")
        hparams = checkpoint.get("hyper_parameters", {})

        # Default hyperparameters inferred from the Cascadia architecture.
        # These match the published model (d_model=512, 9 layers, 8 heads).
        # The combiner weight shape in the checkpoint confirms d_model.
        defaults = {
            "d_model": 512,
            "n_layers": 9,
            "n_head": 8,
        ...
        # Build constructor kwargs: defaults < checkpoint hparams < overrides
        constructor_kwargs = dict(defaults)
        for key in defaults:
            if key in hparams:
                constructor_kwargs[key] = hparams[key]
        ...
        # Load old state dict with special handling for combiner
        ...
        for name, param in old_state.items():
            ...
            if "combiner.weight" in name and param.shape != new_state[name].shape:
                # Old: (d_model, 3*d_model) -> New: (d_model, 4*d_model)
                new_state[name].zero_()
                new_state[name][:, :param.shape[1]].copy_(param)
            else:
