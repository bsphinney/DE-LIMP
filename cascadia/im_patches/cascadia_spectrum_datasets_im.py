...
class SpectrumDataset(Dataset):
    ...
    def add_file(self, ms_data_file: PathLike) -> None:
        ...
        # im_arrays: present for AsfParser, need zeros for other parsers
        if hasattr(parser, 'im_arrays') and len(parser.im_arrays) > 0:
            spectra["im_array"] = parser.im_arrays
        else:
            spectra["im_array"] = np.zeros(parser.n_peaks, dtype=np.float32)
    ...
    def get_spectrum(self, idx: int) -> MassSpectrum:
        ...
        # Load im_array if present in the HDF5 spectrum dtype
        im_data = None
        if "im_array" in spectrum.dtype.names:
            im_arr = np.array(spectrum["im_array"])
            # Only pass IM if there are nonzero values
            if np.any(im_arr != 0):
                im_data = im_arr
...
def _collate_fn(
    ...
    Returns
    -------
    spectra : torch.Tensor of shape (batch_size, n_peaks, 4 or 5)
        The mass spectra to sequence, where ``X[:, :, 0]`` are the m/z
        values and ``X[:, :, 1]`` are their associated intensities.
    precursors : torch.Tensor of shape (batch_size, 2)
        The precursor mass and charge state.
