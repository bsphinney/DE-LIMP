...
class MassSpectrum(MsmsSpectrum):
    ...
    im: array of shape (n_peaks,), optional
        Per-peak ion mobility (1/K0) values. Used for timsTOF
        dia-PASEF data where each fragment has an associated IM value.
    """
    ...
    def to_tensor(self) -> torch.tensor:
        """Combine the m/z, intensity, rt, level, and optionally im arrays into a single tensor.

        Returns
        -------
        torch.tensor of shape (n_peaks, 4) or (n_peaks, 5)
            The mass spectrum information. 5 columns when ion mobility is present.
        """
        arrays = [self.mz, self.intensity, self.rt, self.level]
        if self.im is not None:
            arrays.append(self.im)
        return torch.tensor(np.vstack(arrays).T)
