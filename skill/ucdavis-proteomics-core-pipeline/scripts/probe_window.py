...
SELECTION_RULE = (
    ...
    "replace a run that logs no radius with the remaining run nearest the median in rank "
    "position; pin the median of the measured radii")
...
def main():
    ...
    if radius is None:
        ...
        sys.exit("Could not read the scan-window radius: " + why + " ("
                 ...
                 "cause above and re-run. --raw is not a way past it: it narrows the cohort the "
                 "checks are applied to, and every run named there still has to clear them, so a "
                 "run left out as damaged or as a blank is left out however it is named.")
