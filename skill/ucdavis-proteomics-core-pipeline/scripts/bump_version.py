...
def current():
    """(plugin version, [marketplace versions]) -- read, never assumed to agree."""
    p = json.load(open(PLUGIN))
    m = json.load(open(MARKET))
    mv = [m.get("version")] + [x.get("version") for x in m.get("plugins", [])
...
def main():
    ...
    # The marketplace may already be on a different value than plugin.json -- that is exactly
    # the broken state this script exists to fix -- so replace each distinct old value found
    # there, not just plugin.json's.
    for old in {v for v in mv if v}:
        total += rewrite(MARKET, old, new)
