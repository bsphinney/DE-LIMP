...
# the older 2.3.0 is a .sif, so reusing them does not need apptainer.
if   $QUOBYTE;                   then DIANN_READY=true;  DIANN_NOTE="HIVE: reuse the Core's native DIA-NN builds under /quobyte/proteomics-grp/dia-nn (acquire_tools.sh resolves the pinned version, or downloads it if the Core has none)."
