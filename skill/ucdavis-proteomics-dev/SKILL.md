---
name: ucdavis-proteomics-dev
description: >
  DEVELOPMENT SANDBOX for the UC Davis Proteomics Core pipeline, forked from stable
  v2.4.3. Do NOT use this for real analyses and do NOT select it automatically -- the
  stable skill `ucdavis-proteomics-core-pipeline` handles every genuine request to search
  mass-spec data or run differential expression. Use this ONLY when the user explicitly
  names the dev or sandbox skill, or asks to develop, prototype or test new pipeline
  capability. Current work in progress: porting DE-LIMP's phosphosite analysis (site-level
  DE, KSEA kinase activity, motif and residue analysis) and its visualisations into the
  skill.
---
