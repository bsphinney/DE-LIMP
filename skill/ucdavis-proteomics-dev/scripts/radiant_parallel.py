...
def main():
    ...
    # This exact directory is the contract between step 2 and step 3, and it DIFFERS
    # by mode. Single-pass: Fulcrum's main search reads <results>/radiant-results.
    # MBR: the plugin builds params as params.library.search.*, and points the FIRST
    # pass at <results>/radiant-firstpass-results — writing the array into
    # radiant-results there would leave it unused AND let pass 2 silently reuse
    # non-MBR results. (Verified against radiant_fulcrum_workflow 2.3.3.)
    rsub = "radiant-firstpass-results" if a.mbr else "radiant-results"
    rdir = os.path.join(results, rsub)
    ...
    # Which pass it applies to depends on the mode -- see the comment on rsub above.
    if a.mbr:
        # pass 1 reuses the array; pass 2 MUST re-search, because its whole purpose is
        # to re-query every run with the library built from pass 1. Leaving pass 2 at
        # true would make it reuse the single-pass results and report "MBR did nothing".
        reuse_block = ['[overrides.search]', 'reuse_existing = false', '',
                       '[overrides.library.search]', 'reuse_existing = true']
    else:
        reuse_block = ['[overrides.search]', 'reuse_existing = true']
    toml = "\n".join([
