        ...
        div(style = "overflow-y: auto; max-height: calc(100vh - 200px);",
          ...
          conditionalPanel(
            ...
            div(style = "background: #f0f7ff; border: 1px solid #b8d4f0; border-radius: 8px; padding: 16px; margin-bottom: 12px; text-align: center;",
              ...
              div(style = "display: inline-flex; gap: 6px; align-items: center; flex-wrap: wrap;",
                ...
                actionButton("load_dda_example",
                  "\U0001F9EC Load Example De Novo Data",
                  class = "btn-info btn-sm",
                  title = "Ocelot wildlife-forensics de novo results (Sage + Casanovo + nr BLAST)")
              )
            )
          ),
            ...
            div(style = "display: flex; align-items: center; gap: 16px; flex-wrap: wrap;",
              div(style = "flex: 0 0 340px;",
                sliderInput("dda_denovo_score_threshold",
                  label = tags$span(icon("sliders-h"), " Casanovo peptide score ≥"),
                  min = -1, max = 1.0, value = -1, step = 0.05, width = "100%"),
                tags$small(style = "color:#6c757d; display:block; margin-top:-4px;",
                  "Casanovo peptide score = (product of per-residue scores) − 1 if the peptide ",
                  "fails precursor-mass closure. So a negative score means the WHOLE peptide's mass ",
                  "is incomplete — NOT that the matched residues are wrong (decoy-validated: score<0 ",
                  "hits still match references 99% at their confident residues). Default −1 shows all; ",
                  "raise only to inspect mass-complete reconstructions.")
              ),
              div(style = "flex: 1; min-width: 200px;",
          ...
          navset_card_tab(
                  ...
                  div(style = "display: flex; gap: 12px; align-items: center; flex-wrap: wrap;",
                    ...
                    div(style = "margin-left: auto;",
                      checkboxInput("dda_exclude_contaminants",
                        "Exclude contaminant proteins", value = FALSE, width = "auto")
                    )
                  )
                ),
            ...
            nav_panel("Target-Decoy FDR", icon = icon("chart-line"),
              div(style = "overflow-y: auto; max-height: calc(100vh - 250px); padding-right:6px;",
                div(style = "display: flex; justify-content: space-between; align-items:center; margin-bottom: 8px;",
                  tags$h4(style="margin:0; color:#2c3e50;", icon("flask-vial"),
                ...
                tags$p(style="color:#555; font-size:13px; margin-bottom:14px;",
                  "De novo peptides are BLASTed against NCBI ", tags$b("nr"), " to identify the species — but ",
                  "what is the false-discovery rate when the organism may be absent from the database? ",
                  "We adapt the ", tags$b("NovoBoard"), " decoy-spectra method (Tran et al. 2024) to the homology ",
                  "search. Here is the whole process, end to end."),

                # ---- step helper styling ----
                tags$style(HTML(".fdr-step{border:1px solid #e3e8ee;border-radius:10px;padding:12px 16px;margin-bottom:14px;background:#fff;}
                  .fdr-num{display:inline-block;width:26px;height:26px;line-height:26px;text-align:center;border-radius:50%;background:#2c3e50;color:#fff;font-weight:700;margin-right:8px;}
                  .fdr-h{font-weight:600;color:#2c3e50;font-size:15px;} .fdr-cap{color:#667;font-size:12.5px;margin-top:6px;}")),

                # STEP 1 — decoy spectrum
                div(class="fdr-step",
                  div(tags$span(class="fdr-num","1"), tags$span(class="fdr-h","Build a decoy spectrum")),
                  tags$p(class="fdr-cap", style="margin-top:8px;",
                    "For every real MS/MS spectrum we keep ~20% of its peaks and replace the other ~80% with ",
                    "peaks drawn at random from the global peak pool — keeping the precursor m/z and charge. ",
                    "The result looks like a real spectrum but encodes ", tags$b("no real peptide.")),
                  tags$img(src = denovo_fdr_spectrum_src,
                           style="width:100%;max-width:860px;display:block;margin:6px auto;")),

                # STEP 2 — sequence both
                div(class="fdr-step",
                  div(tags$span(class="fdr-num","2"), tags$span(class="fdr-h","Sequence real and decoy spectra with Casanovo")),
                  tags$p(class="fdr-cap", style="margin-top:8px;",
                    "The identical Casanovo model and settings run on both sets, giving a population of ",
                    tags$b("real"), " de novo peptides and a matched population of ", tags$b("decoy"),
                    " de novo peptides — a true 1:1 null with the same number of queries.")),

                # STEP 3 + 4 — BLAST both, compete
                div(class="fdr-step",
                  div(tags$span(class="fdr-num","3"), tags$span(class="fdr-h",
                      "BLAST both against nr, then compete: FDR = decoy ÷ real")),
                  tags$p(class="fdr-cap", style="margin-top:8px;",
                    "Both populations are BLASTed against nr. Real peptides hit far more often than decoy ",
                    "peptides at every Casanovo score; the cumulative ", tags$b("decoy ÷ real"),
                    " hit ratio is the FDR. Worked example below (ocelot, ", tags$i("Leopardus pardalis"),
                    ", 9 runs):"),
                  tags$img(src = denovo_fdr_competition_src,
                           style="width:100%;max-width:980px;display:block;margin:6px auto;")),

                # STEP 5 — the gate
                div(class="fdr-step", style="background:#fff8e1;border-color:#f9a825;",
                  div(tags$span(class="fdr-num", style="background:#f9a825;","!"),
                      tags$span(class="fdr-h","A clean FDR is necessary but NOT sufficient for a species call")),
                  tags$p(class="fdr-cap", style="margin-top:8px; color:#5d4037;",
                    "The decoy-spectra FDR only controls ", tags$i("chance homology"), ". Two further errors need ",
                    "their own controls: (1) de novo ", tags$b("sequence"), " error — gate with the Casanovo ",
                    "confidence slider (≈21% error at conf ≥0.95, ≈12% at ≥0.99); (2) ", tags$b("species"),
                    " mis-assignment from conserved peptides — use the ", tags$b("Species (LCA)"), " pane ",
                    "(lowest common ancestor across all hits, not the single best hit). ",
                    "Full validation: docs/DENOVO_FDR_VALIDATION.md.")),

                tags$hr(),
                # ---- LIVE per-dataset FDR for the loaded data (recommended control) ----
                div(style="margin-top:6px;",
                  tags$h4(style="color:#2c3e50; margin-bottom:4px;", icon("gauge-high"),
                          " Live FDR for this dataset (bitscore-calibrated)"),
                  tags$p(style="color:#555; font-size:13px;",
                    "The recommended control: rank the DIAMOND hits by BLAST ", tags$b("bitscore"),
                    " against the decoy-spectra null and hold the FDR at 1%. We rank by bitscore — ",
                    tags$b("not"), " by Casanovo score — because on the ocelot validation the Casanovo ",
                    "score barely separates real from decoy spectra (only ~171 peptides clear 1% FDR when ",
                    "gated on it, vs ~5,630 when gated on bitscore), so it makes a poor threshold."),
                  radioButtons("denovo_fdr_calib_mode", "Calibrate FDR against:",
                    choices = c("Decoy spectra (NovoBoard)" = "spectra",
                                "Decoy database (reversed)" = "database"),
                    selected = "spectra", inline = TRUE),
                  tags$p(style="color:#8a909a; font-size:12px; margin:-4px 0 8px;",
                    "Decoy spectra = the conservative null that also controls de novo sequencing error. ",
                    "Decoy database (reversed sequences) controls only chance database matches and needs the ",
                    "forward/reversed BLAST FDR job to have been run. Toggle to compare the two and cross-reference with the Sage IDs."),
                  div(id="denovo_bitscore_fdr_callout", style="margin:8px 0;"),
                  div(style="display:flex; flex-wrap:wrap; gap:10px;",
                    div(style="flex:1 1 360px; min-width:320px;",
                      plotlyOutput("denovo_bitscore_fdr_curve", height="300px")),
                    div(style="flex:1 1 360px; min-width:320px;",
                      plotlyOutput("denovo_bitscore_fdr_threshold", height="300px"))),
                  div(style="margin-top:10px;",
                    plotlyOutput("denovo_bitscore_fdr_bylength", height="280px")),
                  tags$p(style="color:#8a909a; font-size:12px; font-style:italic; margin-top:8px;",
                    "Decoy-spectra null (use scramble FRAC ≥ 0.8). Two decoy types calibrate differently — ",
                    "the decoy-spectra null is the more conservative/correct one for de novo data because it ",
                    "also controls de novo sequencing error. Full method + validation: ",
                    "docs/denovo_decoy_method.html and docs/DENOVO_FDR_VALIDATION.md.")
                )
              )),
            ...
            nav_panel("Proteins", icon = icon("diagram-project"),
              div(style = "overflow-y: auto; max-height: calc(100vh - 250px);",
                div(style = "background: #eef4fb; padding: 12px; border-radius: 8px; margin-bottom: 12px;",
                  tags$p(style = "margin: 0; color: #1a3c5e; font-size: 13px;",
                    icon("diagram-project"),
                    " The de novo peptides assembled into ", tags$b("proteins"),
                    " by the same parsimony model FragPipe (ProteinProphet) and IDPicker use — ",
                    "the minimal protein set explaining the peptides, with ", tags$b("razor"),
                    " peptide assignment and indistinguishable-protein grouping. ",
                    "Click a protein to pop out a per-residue coverage map (reference vs de novo, ",
                    "full-screen-able) that colours amino-acid substitutions by Casanovo confidence. ",
                    "Honours the confidence slider above.")),
                div(style = "display: flex; justify-content: flex-end; gap: 8px; margin: 8px 0;",
                  actionButton("denovo_protein_info_btn", icon("question-circle"),
                    title = "How protein inference works", class = "btn-outline-info btn-sm"),
                  downloadButton("dda_protein_download", "Export protein groups (CSV)",
                                 class = "btn-outline-success btn-sm")),
                DT::DTOutput("denovo_protein_table")
              ))
          )
        )
      ),
