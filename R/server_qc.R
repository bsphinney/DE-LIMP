...
server_qc <- function(input, output, session, values) {
  ...
  generate_norm_diagnostic_plot <- reactive({
    ...
    # ── DDA mode short-circuit (cascadia-denovo addition): pre/post norm view
    # using MaxLFQ matrix; returns early before the DIA pipeline checks below.
    ...
    # ── Non-DDA: MaxLFQ + limma OR DPC-Quant pipeline ──
