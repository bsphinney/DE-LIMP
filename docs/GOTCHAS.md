...
| AI policy is "local" on a publicly reachable server | Public vs local is detected from HF's `SPACE_ID` only. On any other shared/public host set **`DELIMP_PUBLIC_DEPLOYMENT=1`** (forces https-only public endpoints, the 180 s timeout cap, no activity-log notes). `=0` forces the local policy (e.g. a private test Space). Unset = `SPACE_ID` detection; any other value fails closed to public. Resolved once in `app.R` by `ai_resolve_public_deployment()`; the startup log prints `AI policy: public/local`. |
| Local LLM on http refused / reached the wrong machine | vLLM, llama.cpp and Ollama serve plain http. On a local install http is accepted to loopback, RFC1918/CGNAT/ULA and `host.docker.internal`-style names (with an unencrypted-key warning). In Docker mode `localhost` is the container — use `http://host.docker.internal:<port>/v1`. |
| Dataset identity taken from `values$diann_search_settings` | Uploads never reset it, so search A's settings/notes survive into report B. For "which search folder is loaded" use `values$loaded_dataset` via `loaded_dataset_output_dir()`; set it with `loaded_dataset_identity()` in search/HPC/session loads and clear it in upload/example loads. It is bound to the loaded samples, so a forgotten clear fails closed. |
...
| **De novo→homology FDR: calibrate by BLAST bitscore, NOT Casanovo score** | The decoy-spectra FDR must rank DIAMOND hits by **bitscore** (or E-value for standard searches), not by Casanovo peptide score. The Casanovo score barely separates real from decoy spectra — on the ocelot data only ~171 of 312,820 peptides clear 1% FDR when gated on Casanovo score, vs ~5,630 when gated on bitscore. Use `build_denovo_bitscore_fdr()` (helpers_dda.R), not the score-bin helpers. The "94–96% hit under EXTRA" is the RAW any-bitscore≥1 rate, NOT FDR-controlled. Full validation: `docs/denovo_decoy_method.html`. |
| **Two decoy types calibrate differently — decoy-spectra is the conservative/correct one** | Decoy-**spectra** (NovoBoard scrambled spectra) controls de novo *sequencing* error + chance matches; decoy-**database** (reversed proteins) controls only chance matches and over-accepts short peptides (at 1% FDR it accepts 6,184 peptides ≤20aa vs the spectra null's 101). For de novo→homology ID use the decoy-spectra null. Recommended: decoy-spectra + EXTRA sensitivity + bitscore @1% FDR + scramble FRAC≥0.8. |
