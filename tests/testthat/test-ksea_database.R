...
test_that("the full PhosphoSitePlus + NetworKIN table is assembled", {
  skip_if_not_installed("KSEAapp")
  db <- load_ksea_database()
  expect_equal(nrow(db), 240749)                       # the file issue #3 asked for
  expect_equal(sum(db$Source == "PhosphoSitePlus"), 9757)
  expect_equal(sum(db$Source == "NetworKIN"), 230992)
})
...
test_that("DE-LIMP ships no PhosphoSitePlus data of its own", {
  ...
  f <- system.file_or_local("data/networkin_kinase_substrate_July2016.csv.gz")
...
test_that("what was actually used is recorded, never silently substituted", {
  ...
  expect_match(attr(load_ksea_database(), "ksea_source"), "NetworKIN 230992")
...
test_that("the file is found from any working directory the app runs in", {
  ...
  expect_true(nzchar(system.file_or_local("data/networkin_kinase_substrate_July2016.csv.gz")))
})
