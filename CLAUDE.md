...
Unreleased (post-v3.10.4): Cascadia de novo integration (feature branch). Audit follow-ups: covariate-name single source of truth (refactor 22 read sites), `<<-` in `add_covariate()` closure refactor, `DIANN_DEFAULTS` constant extraction in `helpers_search.R`.
